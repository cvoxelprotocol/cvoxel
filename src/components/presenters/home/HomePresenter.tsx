        ...
        <div className="flex-none w-full">
            ...
            <CVoxelsPresenter>
              {!txLoading &&
                (!CVoxelsRecords.content?.cVoxels ||
                  CVoxelsRecords.content?.cVoxels.length === 0) && (
          ...
          >
            <div className="w-full max-w-[820px] text-center mx-auto cursor-pointer h-screen overflow-y-scroll">
              ...
              {!offchainLoading &&
                potentialTxes.map((tx) => (
                  <>
                    <div
                      key={tx.hash}
                      className="rounded-lg shadow-lg bg-white mt-3"
                      onClick={() => selectTx(tx)}
                    >
                      <p className="py-2 ">
                        {" "}
                        {account?.toLowerCase() === tx.from.toLowerCase()
                          ? "You're payer"
                          : "You're payee"}
                        / {formatBigNumber(tx.value, 8)}{" "}
                        {tx.tokenSymbol ? tx.tokenSymbol : "ETH"} / {tx.hash} /{" "}
                        {tx.from}{" "}
                      </p>
                    </div>
                    {selectedTx && selectedTx?.hash === tx.hash && (
                      <>
                        {selectedOffchainItem ? (
                          <div
                            key={`${tx.hash}_detail`}
                            className="w-full h-fit bg-white text-left"
                          >
                            ...
                            <div className="mx-2 md:mx-9">
                              <p className="w-full my-1 py-1 px-6 border rounded-full text-xs md:text-sm">
                                {selectedOffchainItem.deliverable ||
                                  "No Deliverable"}
                              </p>
                            </div>
                              ...
                              <>
                                {connection.status === "connected" ? (
                                  ...
                                  <div className="w-full text-center py-4 shadow-lg bg-white">
                                    <p className="text-xs md:text-lg text-primary font-bold">
                                      Please Connect Ceramic Account to Create
                                      CVoxel
                                    </p>
                ...
                offchainMetaList
                  ...
                  .map((tx) => {
                    return (
                      <div
                        key={tx.txHash}
                        className="rounded-lg shadow-lg bg-white mt-3 py-2"
                      >
                        <p className="py-2 ">
                          {" "}
                          {formatBigNumber(tx.value, 8)}{" "}
