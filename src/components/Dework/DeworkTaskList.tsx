...
export const DeworkTaskList: FC = () => {
  ...
  const storeAll = () => {
    ...
    if (!isSelectedAll) {
      const alTask = onlyTasksWithGenre
        .map((d) => d.taskId)
        ...
        <div className="p-4 sm:flex items-center justify-between absolute top-0 right-0 left-0 z-50 bg-gray-100 dark:bg-card">
          ...
          <div className="flex space-x-4 items-center justify-end text-xs sm:text-lg text-light-on-primary-container dark:text-dark-on-error-container">
            ...
            <div className="flex items-center ">
              <div
                className="relative inline-block w-14 mr-2 align-middle select-none transition duration-200 ease-in"
                onClick={() => storeAll()}
              >
                <span
                  className={clsx(
                    "absolute block w-8 h-8 rounded-full bg-white border-4 appearance-none cursor-pointer",
                    isSelectedAll ? "right-0 bg-primary-300" : "right-1/2"
                  )}
                />
              ...
              <span
                className={clsx(
                  "font-bold",
                  isSelectedAll ? "text-primary-300" : ""
                )}
              >
                Select All
              </span>
          ...
          <div className="w-full relative space-y-1 border-light-on-primary-container dark:border-dark-on-primary-container overflow-y-scroll hidden-scrollbar bg-light-surface-1 dark:bg-dark-surface-1">
            {sortedTasks &&
              sortedTasks.map((item) => {
                return <DeworkTaskItem item={item} key={item.taskId} />;
              })}
          </div>
        </div>
      </div>
