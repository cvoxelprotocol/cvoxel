...
const HomePage:NextPage = () => {
  ...
  const [currentStatus, setCurrentStatus] = useState<"disconnected" | "connecting" | "failed" | "connected">(connection.status)

  useEffect(() => {
    if(connection.status==="connected" && did && currentStatus !== "connected" ) {
      ...
      <div className="flex flex-col items-center w-full h-full pb-12">
        <div className="w-[320px] sm:w-[500px] mt-24">
            <form className="w-full flex justify-center items-center" onSubmit={handleSubmit(onSubmit)}>
                <input className="w-full my-1 py-2 px-6 border rounded-full shadow-xl text-xs md:text-sm hover:outline-secondary focus:outline-secondary" placeholder={'Search DID or ethereum address..'} {...register("address", {required:'Please enter DID or ethereum address'})} />
                <button className="flex items-center justify-center" type="submit">
                    <FontAwesomeIcon className="w-4 h-4 ml-2" icon={faSearch} color={'#EFA9E0'}/>
                </button>
            </form>
        </div>
